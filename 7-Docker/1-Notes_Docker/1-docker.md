...
3. **Run MongoDB in Detached Mode with Port Forwarding**  
   ...
   docker run -d -p 3000:3000 --name nodeapp 1f100a464d1c
...
To assign a name to a container, you can use the `--name` flag with the `docker run` command. Here's an example:
...
- `--name my-container-name` specifies the desired name for your container.
- `my-image` is the name of the Docker image you're using to start the container.

Once the container is created, you can reference it by its name instead of the automatically generated container ID.

If you want to rename an already existing container, you can use the `docker rename` command:
...
Let me explain these Docker commands briefly:

1. **`docker attach`**:

   - **Purpose**: This command connects your terminal's standard input, output, and error streams to a running container. It lets you interact with the container as if it were running directly on your system.
   - **Use Case**: When you need to view logs or interact with the container's processes directly.
   - **Example**:
     ```bash
     docker attach <container_id>
     ```
     Note: Using `docker attach` will attach to all the container's output, so detaching might require `Ctrl + P + Q`.

2. **`docker commit`**:
   - **Purpose**: It creates a new image from the current state of a running or stopped container. This is helpful if you’ve made changes inside the container and want to save them for reuse.
   - **Use Case**: When you customize a container (e.g., install new software) and want to save those changes as a new image.
   - **Example**:
     ```bash
     docker commit <container_id> <new_image_name>
     ```

Both commands have specific roles in working with containers. Do you plan to test either of them? 😊
