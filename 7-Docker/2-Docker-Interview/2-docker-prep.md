...
- A container is a lightweight, portable, and isolated environment that runs applications and their dependencies without needing a separate OS. Containers share the host OS kernel, making them faster and more efficient than virtual machines (VMs).

### **0.1 What is containerization?**

- Containerization is the packaging of software code with just the operating system (OS) libraries and dependencies required to run the code to create a single lightweight executable—called as container—that runs consistently on any infrastructure.

- concept came in 2008
...
- Podman
  ...
  Namespaces create isolated environments so that processes in one namespace can't see or interact with processes in another.

  it controls the PID,network,mount,user

  -2. cgroups (Resource Control)
  Control Groups (cgroups) limit and allocate CPU, memory, disk I/O, and network bandwidth for containers.
...
- **Answer**:
  ...
  - Example:
    ...
    CMD ["node", "index.js"] we can override the file docker run img "main.js"
    ```
...
// vms vs Containers
VMs:  
[Hardware] → [Hypervisor] → [VM1 OS + App] | [VM2 OS + App]

Containers:  
[Hardware] → [Host OS] → [Container Engine] → [App1] | [App2]
